...
public class SecurityConfig {
    ...
    public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        // 커스텀 consentPage 설정 삭제 (기본 Consent 창 사용)
        ...
        http.exceptionHandling(exceptions -> exceptions
                .defaultAuthenticationEntryPointFor(
                        new LoginUrlAuthenticationEntryPoint("/login"), // 스프링 시큐리티 기본 로그인 URL로 이동
    ...
    public SecurityFilterChain developerFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                // 커스텀 폼 로그인 설정 제거 및 기본값 적용
                .formLogin(Customizer.withDefaults())
                .logout(logout -> logout
                        .logoutUrl("/developer/logout")
                        .logoutSuccessUrl("/login?logout") // 기본 로그인 페이지 로그아웃 파라미터
                        .permitAll()
                )
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .formLogin(form -> form
                        .loginPage("/login")
                        // 추가 및 변경: 이전 요청(authorize)을 기억했다가 성공 시 거기로 보냄
